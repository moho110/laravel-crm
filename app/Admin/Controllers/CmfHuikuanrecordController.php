...
use App\Models\CmfHuikuanrecord;
...
class CmfHuikuanrecordController extends AdminController {

	protected $title = "汇款记录";
    ...
    protected function grid() {
                $grid = new Grid(new CmfHuikuanrecord());
        ...
        {
                $form = new Form(new CmfHuikuanrecord());
                $form->select('customerid', __('客户ID'))
                ->options(CmfCustomer::pluck('supplyname','id'))
                ->creationRules('required|max:200|unique:cmf_huikuanrecord',
                ...
                ->creationRules('required',['required' => '此项不能为空']);
                $form->text('guanlianplanid', __('关联计划ID'))
                ->creationRules('required|max:200',
                        ['required' => '此项不能为空','max' =>'不能大于200个字符']);
                $form->textarea('oddment', __('零碎事务'))
                ->creationRules('required|max:200',
                        ['required' => '此项不能为空','max' =>'不能大于200个字符']);
                return $form;
        }
        ...
        {
        $show = new Show(CmfHuikuanrecord::findOrFail($id));
