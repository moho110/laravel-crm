...
use App\Models\CmfFixedassetbaofei;
...
class CmfFixedassetbaofeiController extends AdminController {

	protected $title = "固定资产报废";
    ...
    protected function grid() {
                $grid = new Grid(new CmfFixedassetbaofei());
                $grid->column('id', __('编号'))->sortable();
                $grid->column('setName', __('资产名称'));
                $grid->column('setNo', __('资产编号'));
                $grid->column('inDepartment', __('所属部门'));
                $grid->column('baofeiTime', __('报费时间'));
                $grid->column('baofeiApplyMan', '报费申请人');
                $grid->paginate(10);
        ...
        {
                $form = new Form(new CmfFixedassetbaofei());
                $form->text('setName', __('资产名称'))
                ->creationRules('required|max:200|unique:cmf_fixedassetbaofei',
                        ['required' => '此项不能为空','max' =>'不能大于200个字符','unique' => '数据已经存在']);
                $form->text('setNo', __('资产编号'))
                ->creationRules('required|max:200',
                        ['required' => '此项不能为空','max' =>'不能大于200个字符']);
                $form->select('inDepartment', __('所在部门'))
                ->options(CmfDepartment::pluck('DEPT_NAME','id'))
                ->creationRules('required|max:200',
                        ['required' => '此项不能为空','max' =>'不能大于200个字符']);
                $form->date('baofeiTime', __('报费时间'))
                ->creationRules('required',['required' => '此项不能为空']);
                $form->text('baofeiApplyMan', __('报费申请人'))
                ->creationRules('required|max:200',
                        ['required' => '此项不能为空','max' =>'不能大于200个字符']);
        ...
        {
        $show = new Show(CmfFixedassetbaofei::findOrFail($id));

        $show->field('id', __('编号'));
        $show->field('setName', __('资产名称'));
        $show->field('setNo', __('资产编号'));
        $show->field('inDepartment', __('所在部门'));
        $show->field('baofeiTime', __('报费时间'));
        $show->field('baofeiApplyMan', __('报费申请人'));
