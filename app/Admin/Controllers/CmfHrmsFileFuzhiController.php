...
use App\Models\CmfHrmsFileFuzhi;
...
class CmfHrmsFileFuzhiController extends AdminController {

	protected $title = "人员复职";
    ...
    protected function grid() {
                $grid = new Grid(new CmfHrmsFileFuzhi());
                $grid->column('id', __('编号'))->sortable();
                $grid->column('workId', __('工号'));
                $grid->column('name', __('名称'));
                $grid->column('inDepartment', __('所在部门'));
                $grid->column('telephone', __('电话号码'));
                $grid->column('recoverDate', '复职日期');
        ...
        {
                $form = new Form(new CmfHrmsFileFuzhi());
                $form->text('workId', __('工号'))
                ->creationRules('required|max:200|unique:cmf_hrms_file_fuzhi',
        ...
        {
        $show = new Show(CmfHrmsFileFuzhi::findOrFail($id));
        ...
        $show->field('recoverDate', __('复职日期'));
