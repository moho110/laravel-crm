...
use App\Models\CmfFixedassetout;
...
class CmfFixedassetoutController extends AdminController {

	protected $title = "固定资产出库";
    ...
    protected function grid() {
                $grid = new Grid(new CmfFixedassetout());
        ...
        {
                $form = new Form(new CmfFixedassetout());
                $form->text('setName', __('资产名称'))
                ->creationRules('required|max:200|unique:cmf_fixedassetout',
                        ['required' => '此项不能为空','max' =>'不能大于200个字符','unique' => '数据已经存在']);
                $form->text('setNo', __('资产编号'))
                ->creationRules('required|max:200',
                        ['required' => '此项不能为空','max' =>'不能大于200个字符']);
                $form->select('inDepartment', __('所属部门'))
                ->options(CmfDepartment::pluck('DEPT_NAME','id'))
                ->creationRules('required|max:200',
                        ['required' => '此项不能为空','max' =>'不能大于200个字符']);
                $form->text('jielingCondition', __('借领条件'))
                ->creationRules('required|max:200',
                        ['required' => '此项不能为空','max' =>'不能大于200个字符']);
                $form->text('jielingMan', __('借领人'))
                ->creationRules('required|max:200',
                        ['required' => '此项不能为空','max' =>'不能大于200个字符']);
        ...
        {
        $show = new Show(CmfFixedassetout::findOrFail($id));

        $show->field('id', __('编号'));
        $show->field('setName', __('资产名称'));
        $show->field('setNo', __('资产编号'));
        $show->field('inDepartment', __('所属部门'));
        $show->field('jielingCondition', __('借领条件'));
        $show->field('jielingMan', __('借领人'));
